. Copy the raw text below directly into your file—it cleans up the Mermaid syntax and ensures every block is properly closed so GitHub renders both diagrams seamlessly:
...
```mermaid
flowchart TD
    subgraph DUAL_PATH ["DUAL-PATH ARCHITECTURE"]
        direction TB
